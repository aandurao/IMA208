    ...
    if lamb == 0:
        for r in range(0, cur.shape[0], brow) : # for each block in the current image, find the best corresponding block in the reference image
            for c in range(0, cur.shape[1], bcol) :
                ...
                # Initialization:
                B_search = ref_extended[r+search:r+brow+search, c+search:c+bcol+search]
                costMin = np.inf
                ...
                for dcol in range(-search, search): # dcol = candidate displacement vector over the columns
                    for drow in range(-search, search): # rcol = candidate displacement vector over the rows
                        ...
                        B_search = ref_extended[search+r+drow:search+r+drow+brow, search+c+dcol:search+c+dcol+bcol]
                        cost = np.linalg.norm(B-B_search)**2
                        
                        if cost < costMin :# Save the results if they are better than the previous ones
                            costMin = cost
                            Rbest = [r+drow, c+dcol]
                            
                mvf[r:r+brow, c:c+bcol, 0] = (Rbest[0]-r)*np.ones((brow, bcol)) # Once the loop is over, save the best row displacement field
                mvf[r:r+brow, c:c+bcol, 1] = (Rbest[1]-c)*np.ones((brow, bcol)) # Once the loop is over, save the best column displacement field
                prediction[r:r+brow,c:c+bcol]= ref_extended[search+Rbest[0]:search+Rbest[0]+brow, search+Rbest[1]:search+Rbest[1]+bcol]
                
    else: # Regularized search
        for r in range(cur.shape[0], brow) : # for each block in the current image, find the best corresponding block in the reference image
            for c in range(cur.shape[1], bcol) :
                # current block selection
                B = cur[r:r+brow, c:c+brow] # Block
                ...
                for dcol in range(-search, search): # dcol = candidate displacement vector over the columns
                    for drow in range(-search, search): # rcol = candidate displacement vector over the rows
                        B_search = ref_extended[search+r+drow:search+r+drow+brow, search+c+dcol:search+c+dcol+bcol]
                        cost = np.linalg.norm(B-B_search)**2 + lamb*np.linalg.norm(np.array([drow, dcol]) - pV) # here, encore the REGULARIZED cost function
                        if cost < costMin : # Save the results if they are better than the previous ones
                            costMin = cost
                            Rbest = [r+drow, r+dcol]
                
                
                mvf[r:r+brow, c:c+bcol, 0] = Rbest[0]*np.ones((brow, bcol)) # Once the loop is over, save the best row displacement field
                mvf[r:r+brow, c:c+bcol, 1] = Rbest[1]*np.ones((brow, bcol)) # Once the loop is over, save the best column displacement field
                prediction[r:r+brow,c:c+bcol]= ref_extended[search+Rbest[0]:search+Rbest[0]+brow, search+Rbest[1]:search+Rbest[1]+bcol]
