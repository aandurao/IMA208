...
def fracMc(ref,mvf,outofbound=20):
    ...
    mc_r_tmp = ...  # Row coordinates, after the movement
    mc_c_tmp = ... # Column coordinates, after the movement
    
    extension = outofbound
    ref = cv2.copyMakeBorder(ref, extension, extension, extension, extension, cv2.BORDER_REFLECT)# Padded image
    
    mc_r = # Row coordinates, after the movement (in the padded image)
    mc_c = mc_c_tmp + extension # Column coordinates, after the movement (in the padded image)
    
    colMeshGrid, rowMeshGrid = ... # Original coordinates, in the padded image
    ...
    motcomp =  griddata(...)# Perform the interpolation after the movement
